...
import { unwrapAssistantMarkdown } from '#/lib/chat-response'
...
function assistantTextFromResponse(response: unknown): string {
  if (typeof response === 'string') {
    return unwrapAssistantMarkdown(response)
  }
  ...
  if (record.success === true) {
    ...
    if (typeof data === 'string') {
      return unwrapAssistantMarkdown(data)
    }
    if (data && typeof data === 'object') {
      const message = (data as Record<string, unknown>).message
      if (typeof message === 'string') {
        return unwrapAssistantMarkdown(message)
      }
    }
    return unwrapAssistantMarkdown(JSON.stringify(response))
  }
  ...
  if (record.success === false) {
    ...
    const errors = Array.isArray(record.errors)
      ...
      : []
    return [`**${message}**`, '', ...errors.map((error) => `- ${error}`)].join(
      '\n',
    )
  }
...
export function ChatPrompt() {
  ...
  const form = useForm({
    ...
    onSubmit: async ({ value, formApi }) => {
      ...
      const history = messages.map(({ role, content }) => ({
        role,
        content:
          role === 'assistant' ? unwrapAssistantMarkdown(content) : content,
      }))
      ...
      try {
        ...
        setMessages((prev) => [
          ...prev,
          {
            id: crypto.randomUUID(),
            role: 'assistant',
            content: assistantTextFromResponse(response),
          },
        ])
      } catch {
  ...
  return (
    <div className="mx-auto flex h-full w-full max-w-2xl flex-1 flex-col justify-between">
      <div
        ref={listRef}
          ...
          messages.map((message) => {
            const displayContent =
              message.role === 'assistant'
                ? unwrapAssistantMarkdown(message.content)
                : message.content
